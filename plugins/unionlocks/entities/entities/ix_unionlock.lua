...
if (SERVER) then
	...
	function ENT:Toggle(client)
		...
		local combine = client:IsCombine()
		local admin = client:Team() == FACTION_ADMIN
		local cwu = client:Team() == FACTION_CWU
		local cru = client:Team() == FACTION_CRU
		local cmu = client:Team() == FACTION_CMU
		

		if !(combine or admin or cwu or cru or cmu) then
			self:DisplayError()
